...
有效载荷数据可以分成多个单独的帧。接收端应该对它们进行缓冲，直到设置好 `fin` 位。因此，可以将字符串“Hello World”发送到 11 个包中，每个包的长度为 6(报头长度)+ 1 字节。控件包不允许分片。但是，规范希望能够处理[交错](https://en.wikipedia.org/wiki/Interleaving_%28data%29)的控制帧。这是 TCP 包以任意顺序到达的情况。
